...
namespace Sea
{
    public class NPCShip
    {
        ...
        public void InstantiateNewSceneObject(State state, Datum.StandingData standingData)
        {
            switch (ShipType)
            {
        ...
        public Datum.IHull Hull;
        public Datum.Standing Standing => RegionalMode switch
        {
            RegionEnum.Aeolian => new Datum.AeolianStanding(),
            RegionEnum.Ionian => new Datum.IonianStanding(),
            RegionEnum.Lydian => new Datum.LydianStanding(),
            RegionEnum.Locrian => new Datum.LocrianStanding(),
            RegionEnum.MixoLydian => new Datum.MixolydianStanding(),
            RegionEnum.Dorian => new Datum.DorianStanding(),
            RegionEnum.Phrygian => new Datum.PhrygianStanding(),
            RegionEnum.Chromatic => new Datum.ChromaticStanding(),
        ...
        public NPCShip(Vector2Int[] path, NPCShipType shipType, Datum.IHull hull, Vector2Int regionalCoordOffset, ShipStats.ShipStats shipStats)
        {
            ShipType = shipType;
            ShipStats = shipStats;
            Hull = hull;
    ...
    {
        public TradeShip(State state, Datum.IHull hull, Datum.StandingData standingData, Datum.Standing standing)
        {
            IShipPrefab obj = hull switch
            {
                Datum.Sloop => Assets.Sloop,
                Datum.Brig => Assets.Brig,
                Datum.Schooner => Assets.Schooner,
                Datum.Cutter => Assets.Cutter,
                // Data.TopRig => Assets.TopRig,
                Datum.Frigate => Assets.Frigate,
                Datum.Barque => Assets.Barque,
    ...
    {
        public BountyShipSO(State currentState, NPCShip ship, Datum.IHull hull)
        {
            IShipPrefab obj = hull switch
            {
                Datum.Sloop => Assets.Sloop,
    ...
    {
        public PirateShip(State currentState, NPCShip ship, Datum.IHull hull)
        {
            IShipPrefab obj = hull switch
            {
                Datum.Sloop => Assets.Sloop,
