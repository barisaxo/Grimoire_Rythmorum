...
namespace Gramophones
{
    public static class GramoSystems
    {
        ...
        public static IFunction ChangeAnswer(this GramoScene scene, Dir dir, IFunction currentAnswer)
        {
            if (scene.GetSpinningBool(scene.CurSelection)) return currentAnswer;

            return currentAnswer switch
            {
                null => dir == Dir.Left ? new Dominant() : new Tonic(),
                Tonic => dir == Dir.Left ? new Dominant() : new Subdominant(),
                Subdominant => dir == Dir.Left ? new Tonic() : new Dominant(),
                Dominant => dir == Dir.Left ? new Subdominant() : new Tonic(),
                _ => throw new System.ArgumentException(currentAnswer.ToString())
            };
        }
        ...
        public static IFunction GetCurrentAnswer(this GramoScene scene)
        {
            return scene.CurSelection switch
            {
                _ when scene.CurSelection == scene.Gramo.AnswerMesh1 => scene.CurAnswers[0],
        ...
        public static void SetAnswer(this GramoScene scene, IFunction answer)
        {
            Debug.Log(nameof(SetAnswer) + " " + scene.CurSelection.name + " " + answer);
            if (scene.CurSelection == scene.Gramo.AnswerMesh1) scene.CurAnswers[0] = answer;
            else if (scene.CurSelection == scene.Gramo.AnswerMesh2) scene.CurAnswers[1] = answer;
        ...
        public static bool CorrectAnswers(this GramoScene scene)
        {
            ...
            {
                Debug.Log(i + " of " + scene.AnswerSheet.Length +
                    ", is null? " + (scene.AnswerSheet[i] is null).ToString() +
                    ", is incorrect?" + !scene.AnswerSheet[i].Equals(scene.CurAnswers[i]) +
                    ", answer sheet: " + scene.AnswerSheet[i].Name +
                    ", your answer: " + scene.CurAnswers[i].Name);
                if (scene.AnswerSheet[i] is null || !scene.AnswerSheet[i].Equals(scene.CurAnswers[i]))
                    return false;
            }
            return true;
        }
        ...
        internal static Material NewAnswerMat(this GramoScene scene, MeshRenderer dial)
        {
            ...
            return scene.GetCurrentAnswer() switch
            {
                Tonic => scene.CurSelection switch
                {
                ...
                Subdominant => scene.CurSelection switch
                {
                ...
                Dominant => scene.CurSelection switch
                {
