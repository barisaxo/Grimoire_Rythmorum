...
public class StarChartPractice_State : State
{
    ...
    {
        Keyboard = Puzzle.NumOfNotes == 1 ?
            new(Puzzle.NumOfNotes) :
    ...
    {
        if (GiveUp.GO.activeInHierarchy)
            SetState(new DialogStart_State(new EndPractice_Dialogue(false, SubsequentState)));
    }
    ...
    private void SubmitClicked()
    {
        ...
        {
            DataManager.Puzzles.AdjustLevel(Puzzle, 1);
            DataManager.Player.AdjustLevel(PuzzleType == PuzzleType.Aural ? new AuralSolved() : new TheorySolved(), 1);
            DisableInput();
    ...
    void Finish()
    {
        SetState(new DialogStart_State(new EndPractice_Dialogue(true, SubsequentState)));
    }
