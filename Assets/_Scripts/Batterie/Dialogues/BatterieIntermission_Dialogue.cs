...
public class BatterieIntermission_Dialogue : Dialogue
{
    ...
    Line Fled => new Line("It's better we run and live to fight another day!", new EndBatterie_State(Pack.SetResultType(BatterieResultType.Fled)))
        .SetSpeakerIcon(Assets.Pino)
        .SetSpeakerName(Pino)
        ;
    ...
    Response LetThemGo => new Response("Let them go.", new EndBatterie_State(Pack.SetResultType(BatterieResultType.NMEscaped)));
}
