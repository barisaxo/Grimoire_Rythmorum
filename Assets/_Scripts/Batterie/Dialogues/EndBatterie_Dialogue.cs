...
public class EndBatterie_Dialogue : Dialogue
{
    ...
    public EndBatterie_Dialogue(int gold, int mats, int rations, int damage, bool map, BatterieResultType result)
    {
        _mats = mats;
        _map = map;
        _gold = gold;
    ...
    Line WonWithOutMap => new Line(Damage + Found + Gold + Mats + Rations, new SeaSceneTest_State())
        .SetSpeakerIcon(Assets.Pino)
        .SetSpeakerName(Pino)
        .FadeToNextState();

    Line MapLine => new Line(GhostShip, new SeaSceneTest_State())
        .SetSpeakerIcon(Assets.Pino)
        .SetSpeakerName(Pino)
        .FadeToNextState();

    Line NMEEscapedLine => new Line("They got away sir!" + Damage, new SeaSceneTest_State())
        .SetSpeakerIcon(Assets.Pino)
        .SetSpeakerName(Pino)
        .FadeToNextState();

    Line NMESurrendered => new Line(Damage + Found + Gold + Mats + Rations, new SeaSceneTest_State())
        .SetSpeakerIcon(Assets.Pino)
        .SetSpeakerName(Pino)
        .FadeToNextState();

    Line SurrenderedToNME => new Line(Damage + Lost + Gold + Mats + Rations, new SeaSceneTest_State())
        .SetSpeakerIcon(Assets.Pino)
        .SetSpeakerName(Pino)
        .FadeToNextState();
    ...
    Line CheatLine2 => new Line("I'm sorry Cap, but acting like that isn't going to get us very far!", new SeaSceneTest_State())
        .SetSpeakerIcon(Assets.Pino)
        .SetSpeakerName(Pino);
