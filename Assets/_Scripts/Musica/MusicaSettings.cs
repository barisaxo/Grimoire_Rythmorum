...
namespace Musica
{
    public class MusicaSettings
    {
        ...
        public MusicaSettings(
            float tempo,
            INote keyCenter,
            Genre genre,
            MusicalScale scale,
            Extension extension,
            IRomanNumeral[] cadence
            )
        {
            Tempo = tempo;
            KeyCenter = keyCenter;
            ...
            Chords = GetChordBytes(Cadence, KeyCenter, Genre, Extension, Instrument.Chords, Tempo);
            Basses = GetChordBytes(Cadence, KeyCenter, Genre, Extension, Instrument.Bass, Tempo);
            ...
            )
        {
            ...
            for (int i = 0; i < newChordBytes.Length; i++)
            {
                newChordBytes[i] = new ChordByte(
                    ...
                    // chordQuality: cadence[i].GetTriad(),
                    keyCenter: keyCenter,
                    genre: genre,
                    axe: axe,
                    tempo: tempo
