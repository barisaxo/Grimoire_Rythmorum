...
namespace Menus
{
    public class SBatterieOptionMenu : IMenu
    {
        public SBatterieOptionMenu(State subsequentState)
        {
            SubsequentState = subsequentState;
        }

        readonly State SubsequentState;
        public IData Data { get; } = Manager.Io.SBatterieOptionData;
        ...
        public string GetDescription => BatterieUnlocked ? string.Empty : Selection.Item switch
        {
            ...
            Rests => "Practice " + (5 - Manager.Io.SBatterieOptionData.GetLevel(new NoRestsOrTies())) + " more previous Batterie to unlock",
            Ties => "Practice " + (5 - Manager.Io.SBatterieOptionData.GetLevel(new Rests())) + " more previous Batterie to unlock",
            RestsAndTies => "Practice " + (5 - Manager.Io.SBatterieOptionData.GetLevel(new Ties())) + " more previous Batterie to unlock",
        ...
        public State GetNextState => BatterieUnlocked ?
                new BatteryTutorial_State(null, GetSpecs(), Manager.Io.SBatterieOptionData, Selection.Item, new MenuState(this), false)
                : null;
        ...
        {
            NoRestsOrTies => Manager.Io.SRhythmCellData.GetLevel(new SES()) > 0,
            Rests => Manager.Io.SBatterieOptionData.GetLevel(new NoRestsOrTies()) >= 5,
            Ties => Manager.Io.SBatterieOptionData.GetLevel(new Rests()) >= 5,
            RestsAndTies => Manager.Io.SBatterieOptionData.GetLevel(new Ties()) >= 5,
            _ => throw new System.Exception(Selection.Item.Name)
        };
