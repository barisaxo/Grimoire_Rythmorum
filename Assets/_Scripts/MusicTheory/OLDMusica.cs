...
namespace Musica
{
    ...
    public static class Musica
    {
        ...
        public static string String(this KeyOf key) => KeyToString(key);
        public static string KeyToString(this KeyOf key) => key switch
        {
            KeyOf.C => "C",
            KeyOf.G => "G",
            KeyOf.D => "D",
        ...
        public static KeyOf RootNote(this ChromaticRomanNumeral romanNumeral, KeyOf key)
        {
            ...
            KeyOf KeyPlusInterval(int interval) => (key + interval) > KeyCount() - 1 ?
                (KeyOf)(key + interval - KeyCount()) :
                key + interval;
            KeyOf KeyCount() => (KeyOf)Enum.GetNames(typeof(KeyOf)).Length;
        }
        ...
        public static KeyOf Transposed(this KeyOf key, KeyOf tKey) =>
            (KeyOf)(((int)key - (int)tKey) < 0 ? (int)key - (int)tKey + 12 : (int)key - (int)tKey);

        public static string ToChordName(this ChromaticRomanNumeral numeral, KeyOf key) => ChordName(numeral, key);
        public static string ChordName(ChromaticRomanNumeral numeral, KeyOf key)
        {
